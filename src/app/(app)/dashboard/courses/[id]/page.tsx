...
export default function EditCoursePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ── AI Video Analysis ──
  const handleAIAnalysis = async (mIndex: number, lIndex: number, videoUrlOrFile?: string | File) => {
    const lesson = modules[mIndex].lessons[lIndex];
    updateLesson(mIndex, lIndex, '_analyzingAI' as any, true);
    
    try {
      // We need a file to analyze. If it's a YouTube ID, we can't easily analyze it with the current File API 
      // setup without downloading. For now, this feature works best with "Direct Uploads".
      // However, we can check if it's a recently uploaded video or a file input.
      
      const fd = new FormData();
      // If the user just uploaded a video, we might have it in a state, but for simplicity, 
      // we'll primarily support this during/after a direct upload or by selecting a file again.
      // For this implementation, we'll prompt a file select ONLY if videoId is missing.
      
      // If we don't have a file object passed, we can't proceed with Gemini File API yet.
      // I'll update the UI to allow "Analyze" after selecting a file.
      
      alert("AI Analysis works best with 'Direct Upload'. Please select the video file to analyze.");
    } catch (err: any) {
      console.error('AI Analysis error:', err);
      setError(err.message || 'AI Analysis failed');
    }
    updateLesson(mIndex, lIndex, '_analyzingAI' as any, false);
  };
                    ...
                    <div className="space-y-4 pl-4 border-l-2 border-[var(--border)] ml-4">
                      {module.lessons.map((lesson, lIndex) => (
                        <LessonEditor
                          ...
                          onAIAnalyze={async (file) => {
                            updateLesson(mIndex, lIndex, '_analyzingAI' as any, true);
                            try {
                              const fd = new FormData();
                              fd.append('file', file);
                              const res = await fetch('/api/admin/analyze-video', { method: 'POST', body: fd });
                              if (!res.ok) throw new Error('AI Analysis failed');
                              const data = await res.json();
                              if (data.title) updateLesson(mIndex, lIndex, 'title', data.title);
                              if (data.description) updateLesson(mIndex, lIndex, 'content', data.description);
                              setSuccess(true);
                              setTimeout(() => setSuccess(false), 2000);
                            } catch (e: any) {
                              setError(e.message);
        ...
        {lesson.type === 'video' && (
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <input 
                type="file" 
                id={`ai-analyze-${lesson.title}`} 
                className="hidden" 
                accept="video/*"
                onChange={(e) => e.target.files?.[0] && onAIAnalyze(e.target.files[0])}
              />
              <button
                type="button"
                onClick={() => document.getElementById(`ai-analyze-${lesson.title}`)?.click()}
                disabled={(lesson as any)._analyzingAI}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[11px] font-black transition-all hover:scale-105 active:scale-95 disabled:opacity-50 border border-[var(--accent)]/30 text-[var(--accent)] bg-[var(--accent)]/5 hover:bg-[var(--accent)]/10"
              >
                {(lesson as any)._analyzingAI ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}
                {(lesson as any)._analyzingAI ? 'Reviewing Video...' : 'Analyze with AI'}
              </button>
            </div>
          </div>
        )}
